...
pub struct HttpStreamableConfig {
    pub addr: String,
    pub session_id: SessionId,
}

pub struct HttpStreamableChannel {
    #[allow(dead_code)]
    pub ctx: Arc<ChannelContext>,
    pub http_streamable_config: HttpStreamableConfig,
}

impl HttpStreamableChannel {
    pub async fn new(
        ...
        Ok(Self {
            ...
            http_streamable_config: config
                .http_streamable_config
                .clone()
                .ok_or_else(|| anyhow!("http_streamable config not found"))?,
        })
    }
}
...
#[async_trait]
impl Channel for HttpStreamableChannel {
    type Client = ();
    ...
    async fn start(
                ...
                post(move |Json(body): Json<ChatRequest>| {
                    ...
                    async move {
                        ...
                        let session_id = match body.session_id {
                            ...
                            None => self_.http_streamable_config.session_id.clone(),
                        };
        ...
        let addr: SocketAddr = self_.http_streamable_config.addr.parse()?;
        let listener = tokio::net::TcpListener::bind(&addr).await?;
        info!("HttpStreamable HTTP channel listening on {}", addr);

        let join_handle = tokio::spawn(async move {
            if let Err(err) = axum::serve(listener, app).await {
                error!("HttpStreamable channel server error: {}", err);
                return Err(anyhow!("HttpStreamable server error: {}", err));
            }
            Ok(())
        });
    ...
    async fn handle_agent_message(
        ...
        while let Some(msg_result) = receiver.recv().await {
            match msg_result {
                Ok(msg) => {
                    info!("HttpStreamableChannel background task message: {:?}", msg.message);
                }
                Err(err) => {
                    error!("HttpStreamableChannel background task error: {}", err);
                }
            }
        }
        Ok(())
    ...
    fn allow_session_ids(&self) -> crate::Result<Vec<&SessionId>> {
        Ok(vec![&self.http_streamable_config.session_id])
    }
}
