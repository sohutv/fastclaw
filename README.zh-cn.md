...
Fastclaw 是一个基于 Rust 的本地 AI Agent，支持 OpenAI-Compatible 模型提供商、多通道交互（CLI、钉钉与微信）、流式输出、会话历史、定时任务执行和工具调用。
...
- 通道支持：
  ...
  - 微信通道（`--channel Wechat`，默认 feature 启用）
- 支持推理流与最终回答流式输出
- 内置工具：
  ...
  - `imagegen`（仅在配置 `[imagegen]` 后可用）
  - `cloud-storage-store`、`cloud-storage-load`、`cloud-storage-del`
    （仅在启用 `cloud_storage_tool` feature 且配置 `[storage]` 后可用）
...
可选：配置 imagegen（`volcengine` feature）：
...
可选：配置对象存储（`volcengine` + `cloud_storage_tool` feature）：
...
可选：配置微信（启用 `channel_wechat_channel` 时）：
...
`db.sqlite` 会在首次执行 `start` 时创建，而不是在 `onboard init-config` 阶段创建。
...
- `onboard init-config` 当前会把 `resources/CRON_TASK.md` 写入 `workspace/USER.md`（覆盖此前写入的 `USER.md` 内容）。
